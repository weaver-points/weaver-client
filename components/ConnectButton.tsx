...
export default function ConnectButton() {
  ...
  const handleConnect = async () => {
    try {
      ...
      const sessionAccount = response?.account;
      if (!sessionAccount) {
  ...
  return (
    <div>
      {" "}
      {isConnected ? (
        <button onClick={handleDisconnect} className=" py-2 px-4 bg-[#787272] ">
          {" "}
          Disconnet wallet{" "}
        </button>
      ) : (
        <button onClick={handleConnect} className=" py-2 px-4 bg-[#787272] ">
          {" "}
          Connect wallet{" "}
        </button>
      )}{" "}
